...
O mecanismo de herança do Java permite apenas que uma classe herde de uma única superclasse (herança simples / *singular inheritance*). Em algumas linguagens de programação, como C++, é possível que uma subclasse herde de múltiplas superclasses (herança múltipla). Como herança múltipla pode gerar alguns problemas (caso, por exemplo, superclasses contenham métodos com os mesmos nomes e parâmetros), ela foi deixada de fora no Java.
