...
Os métodos em Java são onde você coloca as operações sobre dados (variáveis) em seu código Java. Em outras palavras, você agrupa operações Java (código) dentro de métodos Java. Os métodos Java devem estar localizados dentro de uma classe Java. Métodos Java são semelhantes ao que é chamado de funções ou procedimentos em outras linguagens de programação (por exemplo, Pascal ou JavaScript).  
...
Aqui está um exemplo simples de método Java:
...
O exemplo acima define um método chamado `writeText` dentro de uma classe chamada `MyClass`. O método recebe um único parâmetro chamado `text`. Esse parâmetro é usado na instrução Java dentro do método. O método não retorna nenhum valor.  
